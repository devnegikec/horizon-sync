...
> **Status**: 📝 **SPECIFICATION — the returns module is not deployed yet.** Every endpoint,
> field and error code below is the agreed contract to be implemented by the returns MVP
> (`R-01` → `R-10` in `INBOUND_EXCEPTION_AND_RETURNS_GAP_ANALYSIS.md` §4.1). Do not wire the
> UI against it until the endpoints answer, but build the screens against these shapes.
...
| Concern                        | Where                                                                             |
| ------------------------------ | --------------------------------------------------------------------------------- |
| Models                         | `app/models/return_registration.py`, `return_receipt_note.py` (new)                |
| Service                        | `app/services/return_service.py` (registration, note, disposition, put-away bridge) |
...
| Permissions                    | `return.read` / `.register` / `.receive` / `.classify` / `.approve` / `.dispose`    |
| Migrations                     | `return_registrations`, `return_registration_items`, `return_receipt_notes`, `return_receipt_note_items`, `return_receipt_note_events` |
| Reuse (no new endpoint)        | `InboundExceptionService.assert_manager`, `BinStockService`, `PutAwayService`, `DocumentNumberingService`, `GET /inbound/exception-reasons` |
