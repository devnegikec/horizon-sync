...
export function QSealFilters({ filters, setFilters }: QSealFiltersProps) {
  return (
    <div className="flex flex-col gap-4 md:flex-row md:items-center rounded-lg border bg-card p-4">
      ...
      <Select
        value={filters.qr_type || 'all'}
        onValueChange={(v) => setFilters((prev) => ({ ...prev, qr_type: v }))}
      >
        ...
        <SelectContent>
          ...
          <SelectItem value="secure_qr_runtime">Secure QR Runtime</SelectItem>
          <SelectItem value="static_qr">Static QR</SelectItem>
        </SelectContent>
      </Select>
    </div>
  );
