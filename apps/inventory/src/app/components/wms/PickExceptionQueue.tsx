...
export function PickExceptionQueue({ warehouseId, refreshKey }: { warehouseId?: string; refreshKey?: number }) {
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      const res = await pickExceptionApi.list(token, 1, 100, {
        severity: filterParam(severity),
        status: filterParam(status),
        warehouse_id: orUndefined(warehouseId),
      });
  ...
  useRefreshOnKey(refreshKey, load);
  ...
  const actionable = (exception: PickException) => exception.status === 'open' || exception.status === 'approved';
  ...
  return (
    <div className="space-y-4">
      ...
      {!loading && exceptions.length === 0 && <p className="rounded-lg border py-8 text-center text-sm text-muted-foreground">No pick exceptions.</p>}
          ...
          <article key={exception.id} className="rounded-lg border p-4">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                {exception.resolution && <p className="mt-2 text-sm text-muted-foreground">Resolution: {exception.resolution}</p>}
              </div>
