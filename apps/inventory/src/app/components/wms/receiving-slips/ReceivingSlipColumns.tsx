...
function ReceivingSlipActionsCell({ slip, onView, onApprove, onReject, onPutAway }: { slip: ReceivingSlip } & ReceivingSlipColumnsOptions) {
    ...
    <div className="flex items-center justify-end gap-2">
      {isPendingReview && (
        <>
          ...
          <Button size="sm"
            variant="outline"
            className="h-7 border-destructive/20 px-2 text-xs text-red-600 hover:!bg-red-600 hover:!text-white"
      ...
      {isPendingPutAway && (
        <Button size="sm"
          variant="outline"
          className="h-7 border-blue-200 px-2 text-xs text-blue-600 hover:!bg-blue-600 hover:!text-white"
...
export function createReceivingSlipColumns({ onView, onApprove, onReject, onPutAway }: ReceivingSlipColumnsOptions): ColumnDef<ReceivingSlip>[] {
  return [
    {
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <ReceivingSlipActionsCell slip={row.original} onView={onView} onApprove={onApprove} onReject={onReject} onPutAway={onPutAway} />
      ),
      enableSorting: false,
    },
  ];
