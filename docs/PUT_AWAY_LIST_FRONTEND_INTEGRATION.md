...
**1. API layer — wms.ts**
- `putAwayApi.generateFromSlip` now accepts optional `{ mode?: 'auto' | 'manual'; worker_id?: string }` and sends it in the POST body (previously it always sent `{}`).

**2. Hook — useWMS.ts**
- `useReceivingSlips.generatePutAway` now accepts `{ mode?, workerId? }` and forwards them to the API, per the resolution order (explicit mode → org default → `auto`).

**3. New component — GeneratePutAwayDialog.tsx**
- A proper generation dialog (replacing the old bare confirmation) with:
  - **Mode** dropdown: `Default (org setting)` / `Automatic` / `Manual` (default omits `mode` so the backend honours `putaway_mode`).
  - **Worker** picker (optional), loaded via `wmsWorkerApi.list` scoped to the slip's warehouse.
  - A hint explaining skipped lines (damaged/rejected/held/quarantined/excess).
  - Success toast surfaces `list.warnings` (e.g. skipped items) returned by the backend.

**4. UI wiring — ReceivingSlipList.tsx**
- The `Put-Away` button (shown on `pending_putaway` slips) now opens `GeneratePutAwayDialog` instead of the old `ConfirmationDialog`.

**5. Settings API — pick-settings.ts**
- Added `getCatalog`, `getEffective`, `upsert`, and `reset` methods backing the documented `putaway_mode` settings endpoints (for the settings editor and org-default behaviour).

TypeScript compiles cleanly (`tsc --noEmit` exit 0).

Note: the existing put-away detail view (`PutAwayView` / `PutAwayDetailDialog`) already handles viewing lists, completing items with bin assignment, and skipping — so no changes were needed there.
