...
| File | Change |
|------|--------|
| qrBlock.types.ts | Added `master_pack_enabled` & `master_pack_size` to `QRBlockCreate`; added `parent_block_id`, `parent_block_download_url`, `parent_block_batch` to `QRBlock` |
| CreateBlockDialog.tsx | Added master pack checkbox + "Items per Master Pack" input + live summary calculation |
| BlockDetailDialog.tsx | Added parent block download card in the completed state |
| BlocksManagement.tsx | Added parent block download in the inline `BlockStatusTracker` banner |

---

### How it works (frontend):

1. **User opens "Generate QR Block" dialog** → sees new "Enable Master Pack (Cascade)" checkbox at the bottom
2. **User checks it** → an "Items per Master Pack" number input appears + a summary card showing:
   ```
   • 10,000 child QR codes
   • 1,000 parent master pack QR codes (10000 ÷ 10)
   • Parents will be cascaded (linked) to their children
   ```
3. **User submits** → API receives `master_pack_enabled: true` and `master_pack_size: 10`
4. **After completion**, the **view dialog** shows two download buttons:
   - "Download Excel" (child block)
   - "Master Pack Parent Block" (parent block) — in a bordered card with the parent batch name

---

### What the backend needs to implement:

When the API receives `master_pack_enabled: true` with `master_pack_size: N`:

1. Create the regular QR block with `quantity` child codes
2. Create `Math.ceil(quantity / N)` parent QSeal nodes with `app_cascade_map: true`
3. Link children to parents in groups of size `N`
4. Return `parent_block_id`, `parent_block_download_url`, and `parent_block_batch` in the `QRBlock` response
